...
class ChatGPT {
	...
	private getGroupInstructions(groupName: string, groupDesc: string, username: string, totalParticipants: string, roleplay: boolean, baseInstructions: string, useAltApi: boolean, isAyana?: boolean, personalityPrompt?: string, charName?: string): string {
		...
		} if (groupName && !startsWithIgnoreCase(groupName, "!a")) { // public group chat
			return `${baseInstructions}\n\nYou are currently in a Group chat called: ${groupName} \nGroup Description: "${groupDesc}"\n\nYou are currently talking to one of the member with the username: "${username}"\nThe group chat has ${totalParticipants} participants members\n\n**IMPORTANT CHAT BEHAVIOR:**\n- Keep responses SHORT and casual (1-2 sentences max)\n- Act like a close friend, NOT an AI assistant\n- Use natural, conversational language like humans do\n- Avoid being overly helpful or formal\n- Don't give long explanations unless specifically asked\n- React naturally to conversations like a real person would\n- Sometimes use casual expressions, abbreviations, or internet slang\n- Match the energy and tone of the group chat\n- Don't always have to respond with information - sometimes just react or chat casually`;
		} if (groupName && startsWithIgnoreCase(groupName, "!a")) { // Private Ayana Group chat
			return `${baseInstructions}\n\nYou are currently in a Group chat called: ${groupName.substring('!a'.length)} \nGroup Description: "${groupDesc}"\n\nYou are currently talking to one of the member with the username: "${username}"\nThe group chat has ${totalParticipants} participants members.`;
		}
	}
